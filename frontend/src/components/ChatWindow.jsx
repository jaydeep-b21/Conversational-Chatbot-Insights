...
function ChatWindow({ sessionId, setSessionId, username, refreshSessions }) {
  ...
  const [typingText, setTypingText] = useState('Generation in progress ');
  ...
  const handleSend = async () => {
    ...
    try {
      const replyData = await sendMessage(sessionId, input, username);

      const botMessage = {
        role: 'assistant',
        message: replyData.response,
        timestamp: new Date().toISOString(),
        source_type: replyData.source_type,
  ...
  const handleRegenerate = async (idx) => {
    ...
    setMessages((prev) => [...prev, prevUserMsg]);
    setLoading(true);
    setShowTyping(true);

    try {
      const replyData = await sendMessage(sessionId, prevUserMsg.message, username);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTypingText(`Generation in progress ${dots[index]}`);
          ...
          <>
            ...
            <div style={styles.inputRow}>
              ...
              <button
                ...
                style={{
                  ...styles.button,
                  ...(isHoveringSend && styles.glowHover)
                }}
